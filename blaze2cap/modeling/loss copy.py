...
class MotionCorrectionLoss(LossBase):
    ...
    4. Canonical MPJPE Loss (FK): Forward Kinematics on Body Rotations -> 3D Position Error.
    ...
    def __init__(self, 
                 ...
                 lambda_contact: float = 0.5, # new contact weight
                 lambda_floor: float = 5.0 , # floor glue to make walk on same plane
                 lambda_tilt: float = 5.0, # tilt correction to prevent falling forward/backward
                 ):
        super().__init__()
    ...
    def _differentiable_fk_canonical(self, body_rot_6d):
        ...
        Args:
            body_rot_6d: Tensor [B, S, 21, 6] (Joints 2-21)
        ...
        # Key change: Use predicted Root rot as the start --
        # index 0 is 
        ...
        global_rots = [root_rot, root_rot]
        global_pos = [root_pos, root_pos]
        
        # Iterate Body Joints (2 to 21)
        for i in range(2, 22):
            ...
            # Map joint index 'i' to body_rot_6d index 'i-2' (offset by -2 because input starts at Joint 2)
            local_rot = rot_mats[:, :, i-2] # (B, S, 3, 3)
    ...
    def forward(self, inputs, targets, mask=None):
        ...
        if isinstance(inputs, tuple):
            ...
            pred_full = torch.cat([root_pred, body_pred], dim=2) # [B, S, 22, 6]
        else:
            pred_full = inputs # Assume already concatenated [B, S, 22, 6]
        ...
        # C. Body Rotation (Local Pose  )
        l_pose_rot = masked_mse(pred_body_rot, target_body_rot)
        
        # D. Canonical MPJPE (FK)
        # Pass only the body orientations (2-21) to FK, as Root is fixed in position and rotation for MPJPE calculation
        # this handles the foxed root internally.
        pos_pred = self._differentiable_fk_canonical(pred_body_rot) # [B, S, 22, 3]
        pos_target = self._differentiable_fk_canonical(target_body_rot) # [B, S, 22, 3]
        l_pose_pos = masked_mse(pos_pred, pos_target)
