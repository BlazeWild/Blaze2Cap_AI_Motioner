"""
Plot Screen X, Y positions reconstructed from deltas (velocities)
"""
import numpy as np
...
SCREEN_POS_START = 12  # Channels 12-13 (not used, we use deltas)
SCREEN_VEL_START = 14  # Channels 14-15 (delta_screen)
...
def reconstruct_from_deltas(data):
    """
    Reconstruct screen positions from velocity deltas.
    Input: (Frames, 27, 18)
    Output: (Frames, 27, 2) - Reconstructed screen positions
    """
    F, J, _ = data.shape
    
    # Extract actual positions (channels 12-13) - centered screen coords
    actual_positions = data[:, :, SCREEN_POS_START:SCREEN_POS_START+2]  # (F, 27, 2)
    
    # Extract velocities (channels 14-15)
    velocities = data[:, :, SCREEN_VEL_START:SCREEN_VEL_START+2]  # (F, 27, 2)
    
    # Extract anchor flags (channel 17)
    anchor_flags = data[:, :, 17]  # (F, 27)
    
    # Initialize positions array
    positions = np.zeros((F, J, 2), dtype=np.float32)
...
def main():
    ...
    print("Reconstructing screen positions from velocity deltas...")
    ...
    ax.set_title('Screen Positions (Reconstructed from Deltas)', fontsize=14, fontweight='bold')
    ax.set_aspect('equal')
