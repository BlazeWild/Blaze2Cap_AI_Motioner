...
CONFIG = {
    ...
    "batch_size": 512,       # L4 24GB handles large batches well
    "num_workers": 8,        # L4 VMs typically have good CPUs
    "lr": 1e-4,
    "weight_decay": 0.01,
    ...
    "use_amp": True,         # Mixed Precision - essential for L4 performance
...
def validate(model, loader, criterion, device, epoch, skeleton_config=None):
    ...
    # Calculate MPJPE (Motion Metric)
    if skeleton_config is None:
        # Default skeleton config (should be replaced with real values)
        # Using TotalCapture 22-joint hierarchy
        skeleton_config = {
            'parents': [0, 0, 1, 2, 3, 4, 5, 6, 5, 8, 9, 10, 5, 12, 13, 14, 0, 16, 17, 0, 19, 20],
            'offsets': torch.ones(22, 3) * 0.1  # Placeholder - should use real bone lengths
        }
        
    metrics = evaluate_motion(full_preds, full_targets, skeleton_config)
    return metrics
