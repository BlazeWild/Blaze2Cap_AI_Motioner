...
def reconstruct_motion_isolated(gt_data):
    """
    Reconstructs motion using ONLY Hip Rotation Deltas.
    Everything else is forced to Rest Pose (Local Rot = Identity).
    Root Position is forced to (0,0,0).
    """
    frames = gt_data.shape[0]
    
    # 1. Extract Hip Rotation Delta (Index 1) -> (F, 6)
    hip_delta_6d = gt_data[:, 1, :]
    hip_delta_mats = cont6d_to_mat(hip_delta_6d) # (F, 3, 3)
    
    # 2. Accumulate Hip Rotation
    # H_curr = H_prev @ Delta
    hip_global_rots = []
    curr_rot = np.eye(3) # Initial Hip Orientation = Identity (1,0,0,0,1,0...)
    ...
    # 3. Perform FK with Rest Pose Body
    # Skeleton Config
    skel = get_totalcapture_skeleton()
    parents = skel['parents'] # List of parent indices
    ...
    for t in range(frames):
        ...
        # ROOT (Joint 0) -> Fixed at (0,0,0), Rotation = Identity
        frame_pos[0] = [0, 0, 0]
        frame_rots[0] = np.eye(3)
        
        # HIPS (Joint 1) -> Pos relative to Root, Rot from Accumulator
        # Global Rot = Hip Accumulator
        frame_rots[1] = hip_global_rots[t]
        
        # Global Pos = ParentPos (0,0,0) + ParentRot (I) @ Offset
        # Note: Offsets are in parent frame.
        # Parent of 1 is 0. Rot of 0 is I.
        frame_pos[1] = frame_pos[0] + frame_rots[0] @ offsets[1]
        
        # BODY (Joints 2-21) -> Fixed Local Rotation = Identity
        for i in range(2, 22):
            p = parents[i]
            local_rot = np.eye(3) # REST POSE
...
class SimpleVisualizer:
    def __init__(self, data):
        ...
        self.ax.set_title("Hip Delta Verification (Rest Pose Body)")
...
def main():
    ...
    print("Reconstructing Motion from Hip Delta Only...")
    reconstructed = reconstruct_motion_isolated(gt_data)
