...
Uses milestone_epoch25.pth for inference
Root position starts at (0,0,0) with delta (0,0,0)
Rest positions calculated from offsets in skeleton_config.py
Fixed scale: -3 to 3 on all axes
"""
...
def preprocess_blazepose(raw_data, window_size=60):
    ...
    # BlazePose parent-child relationships
...
def rotation_6d_to_matrix(d6):
    """Convert 6D rotation to 3x3 matrix using Gram-Schmidt"""
    a1 = d6[0:3]
    a2 = d6[3:6]
...
def forward_kinematics(gt_6d, offsets, parents):
    ...
    CORRECT APPROACH:
    1. Calculate bone lengths from offsets (these are fixed anatomical lengths)
    2. Apply predicted 6D rotations to get bone directions
    3. Position = Parent Position + (Rotation @ Bone_Direction) * Bone_Length
    
    gt_6d format:
    - Joint 0: [pos_x, pos_y, pos_z, 0, 0, 0] - accumulated root position
    - Joint 1: [r00, r01, r02, r10, r11, r12] - accumulated root rotation (first 2 columns of rotation matrix)
    ...
    # Pre-calculate bone lengths from offsets
    bone_lengths = np.zeros(num_joints)
    for j in range(num_joints):
        offset = offsets[j].numpy()
        bone_lengths[j] = np.linalg.norm(offset)
    
    # Joint 0: Root position (accumulated)
    positions[0] = gt_6d[0, 0:3]
    rotations[0] = np.eye(3)  # Not used, joint 1 has the actual root rotation
    
    # Joint 1: Root rotation (accumulated, stored as first 2 columns of rotation matrix)
    # Reconstruct the full rotation matrix from the 6D representation
    col0 = gt_6d[1, 0:3]
    col1 = gt_6d[1, 3:6]
    col2 = np.cross(col0, col1)
    rotations[1] = np.stack([col0, col1, col2], axis=1)
    positions[1] = positions[0]  # Same position as root
    
    # Joints 2-21: Child joints with local rotations
    for j in range(2, num_joints):
        parent_idx = parents[j]
        
        # Get the predicted local rotation
        ...
        # Get bone direction from offset (normalized)
        offset = offsets[j].numpy()
        if bone_lengths[j] > 1e-6:
            bone_direction = offset / bone_lengths[j]
        else:
            bone_direction = np.array([0.0, 0.0, 0.0])
        
        # Apply parent rotation to bone direction and scale by bone length
        bone_vector = rotations[parent_idx] @ bone_direction * bone_lengths[j]
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Interactive 3D Skeleton Viewer')
    parser.add_argument('--input', type=str, default=None,
                        help='Path to input .npy file')
    parser.add_argument('--checkpoint', type=str, default='checkpoints/milestone_epoch25.pth',
    ...
    # Default input path if not specified
    if args.input is None:
        input_path = os.path.join(script_dir, "..", "training_dataset_both_in_out", 
                                  "blaze_augmented", "S1", "acting1", "cam1", 
    ...
    print(f"  Root shape: {root_pred.shape}, Body shape: {body_pred.shape}")
    print()
    
    # Accumulate root motion (deltas -> absolute)
    # Root position starts at (0,0,0), delta starts at (0,0,0)
    print("Accumulating root motion (position + rotation deltas)...")
    print("Root position starts at (0, 0, 0)")
    print("Delta starts at (0, 0, 0)")
    print()
    
    accumulated_predictions = predictions.copy()
    
    # Initialize at origin
    accumulated_root_pos = np.zeros(3, dtype=np.float32)
    accumulated_root_rot = np.eye(3, dtype=np.float32)
    ...
    for i in range(len(predictions)):
        ...
        # Convert back to 6D (use accumulated rotation)
    ...
    print("Rest positions calculated from offsets in skeleton_config.py")
    print()
    ...
    print("Fixed scale: -2 to 2 on all axes")
    ...
    # Set FIXED axis limits: -2 to 2 on all axes
